...
const primaryReplicaUnavailableMessage = "rethinkdb: Cannot perform write: primary replica for shard not available"
...
func expectWaitForWrites(mock *r.Mock, times int) *r.MockQuery {
...
func TestExecWithRetryRetriesPrimaryReplicaFailure(t *testing.T) {
	conn, mock := newRetryTestConnection(3)
	waitQuery := expectWaitForWrites(mock, 1)
...
func TestExecWithRetryExhaustsPrimaryReplicaRetries(t *testing.T) {
	conn, mock := newRetryTestConnection(3)
	waitQuery := expectWaitForWrites(mock, 3)
	wantErr := opFailedTestError(primaryReplicaUnavailableMessage)
	attempts := 0
	...
	if attempts != 4 {
		t.Errorf("query attempts = %d, want 4", attempts)
	}
	if !strings.Contains(err.Error(), "after 4 of 4 attempts") {
		t.Errorf("execWithRetry() error = %q, want attempt count", err)
	}
	if !errors.Is(err, wantErr) {
		t.Errorf("execWithRetry() error = %v, want wrapped error %v", err, wantErr)
	}
	mock.AssertNumberOfExecutions(t, waitQuery, 3)
	mock.AssertExpectations(t)
}
...
func TestExecWithRetryDoesNotRetryUnrelatedErrors(t *testing.T) {
	tests := []struct {
		...
		{
			name: "unrelated operation failure",
			err:  opFailedTestError("rethinkdb: Table is not available"),
		},
		{
			name: "ordinary error",
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			conn, _ := newRetryTestConnection(3)
			attempts := 0
			...
			if !strings.Contains(err.Error(), "after 1 of 4 attempts") {
				t.Errorf("execWithRetry() error = %q, want attempt count", err)
			}
		})
	}
...
func TestRetryActionFor(t *testing.T) {
	tests := []struct {
		...
		{
			name: "query timeout",
			err:  fmt.Errorf("wrapped: %w", r.ErrQueryTimeout),
			want: waitForWrites,
		},
		{
			name: "closed connection",
			err:  fmt.Errorf("wrapped: %w", r.ErrConnectionClosed),
			want: reconnect,
		},
		{
			name: "primary replica unavailable",
			err:  opFailedTestError(primaryReplicaUnavailableMessage),
			want: waitForWrites,
		},
		{
			name: "case insensitive primary replica message",
			err:  opFailedTestError("rethinkdb: PRIMARY REPLICA unavailable"),
			want: waitForWrites,
		},
		{
			name: "primary replica message without operation failure type",
			err:  errors.New(primaryReplicaUnavailableMessage),
			want: doNotRetry,
		},
		{
			name: "unrelated operation failure",
			err:  opFailedTestError("rethinkdb: Database does not exist"),
			want: doNotRetry,
		},
	}
