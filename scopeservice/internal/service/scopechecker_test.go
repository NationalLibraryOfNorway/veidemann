...
func TestFullScript(t *testing.T) {
	...
	tests := []struct {
		...
		{"apexSeedIncludesNumberedWww",
			defaultScopeScript,
			newQUri("http://www2.foo.bar/aa", "http://foo.bar/", "RL"),
			false,
			&scopecheckerV1.ScopeCheckResponse{
				Evaluation: scopecheckerV1.ScopeCheckResponse_INCLUDE,
			}},
		{"canonicalizedUppercaseWww",
			defaultScopeScript,
			newQUri("HTTP://WWW2.FOO.BAR/aa", "http://foo.bar/", "RL"),
			false,
			&scopecheckerV1.ScopeCheckResponse{
		...
		{"numberedWwwSeedIncludesApex",
			defaultScopeScript,
			newQUri("http://foo.bar/aa", "http://www2.foo.bar/", "RL"),
			false,
			&scopecheckerV1.ScopeCheckResponse{
				Evaluation: scopecheckerV1.ScopeCheckResponse_INCLUDE,
			}},
		{"numberedWwwSeedIncludesSiblingSubdomain",
			defaultScopeScript,
			newQUri("http://news.foo.bar/aa", "http://www2.foo.bar/", "RL"),
			false,
			&scopecheckerV1.ScopeCheckResponse{
				Evaluation: scopecheckerV1.ScopeCheckResponse_INCLUDE,
			}},
		{"numberedWwwAltSeedIncludesApex",
			defaultScopeScript,
			setAnnotation(newQUri("http://alt.host.com/aa", "http://foo.bar/", "RL"), "scope_altSeeds", "http://www1.alt.host.com/"),
			false,
			&scopecheckerV1.ScopeCheckResponse{
				Evaluation: scopecheckerV1.ScopeCheckResponse_INCLUDE,
			}},
		{"wwwSeedRemainsExactWhenSubdomainsDisabled",
			defaultScopeScript,
			setAnnotation(newQUri("http://foo.bar/aa", "http://www.foo.bar/", "RL"), "scope_includeSubdomains", "False"),
			false,
			&scopecheckerV1.ScopeCheckResponse{
				Evaluation:    scopecheckerV1.ScopeCheckResponse_EXCLUDE,
				ExcludeReason: script.Blocked.AsInt32(),
